...
namespace :load do
  task :defaults do
    ...
    set :slack_msg_starting, ->{ "#{ENV['CIRCLE_USERNAME'] || ENV['USER'] || ENV['USERNAME']} has started deploying branch #{fetch :branch} of #{fetch :application} to #{fetch :stage, 'an unknown stage'}." }
    set :slack_msg_finished, ->{ "#{ENV['CIRCLE_USERNAME'] || ENV['USER'] || ENV['USERNAME']} has finished deploying branch #{fetch :branch} of #{fetch :application} to #{fetch :stage, 'an unknown stage'}." }
    set :slack_msg_failed,   ->{ "*ERROR!* #{ENV['CIRCLE_USERNAME'] || ENV['USER'] || ENV['USERNAME']} failed to deploy branch #{fetch :branch} of #{fetch :application} to #{fetch :stage, 'an unknown stage'}." }
  end
end
